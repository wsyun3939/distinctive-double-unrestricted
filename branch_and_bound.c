...
int branch_and_bound(IntDequeue *q, int UB, int UB_cur, int LB, direction Dir,int DstDeque) {
	...
	for(i=0;i<STACK;i++){
		dir=q[i].dir;
		if(dir==both) dir= q[i].que[q[i].front] < q[i].que[(q[i].front + q[i].num - 1) % q[i].max] ? lower : upper;
		...
		if(i==0){ //ターゲットデックにおける積み替え
		if (SecondPosition == q[0].min_idx) { //積み替え後，取り出しが可能な場合
			DirNext = both;
				...
				total+=end-start;
				ans=branch_and_bound(q_temp, UB, UB_cur, LB_temp, DirNext,DstDeque);
				if (ans!=0 && ans!=-1) {
				...
				Array_print(q_temp);
						...
						total+=end-start;
						ans=branch_and_bound(q_temp, UB, UB_cur, LB_temp, DirNext,j);
						if(ans!=0 && ans!=-1){
							Array_terminate(q_temp);
							free(q_temp);
							return MinRelocation;
		...
		for(j=STACK-1;j>0;j--){
			if(BG_index[i][j]==0 && !(i==DstDeque && dir==Dir)){
				...
				total+=end-start;
				ans=branch_and_bound(q, UB, UB_cur, LB_temp, lower,j);
				if(ans!=0 && ans!=-1){
					return MinRelocation;
				}
				else if(ans==-1){
	...
	for(i=0;i<STACK;i++){
		...
		if (SecondPosition == q[0].min_idx) { //積み替え後，取り出しが可能な場合
			DirNext = both;
				...
				Array_print(q_temp);
						...
						total+=end-start;
						ans=branch_and_bound(q_temp, UB, UB_cur, LB_temp+1, DirNext,j);
						if(ans!=0 && ans!=-1){
							Array_terminate(q_temp);
							free(q_temp);
							return MinRelocation;
		...
		for(j=STACK-1;j>=0;j--){
			if(BG_index[i][j]==1 && !(i==DstDeque && dir==Dir)){
				...
				total+=end-start;
				ans=branch_and_bound(q, UB, UB_cur, LB_temp+1, lower,j);
				if(ans!=0 && ans!=-1){
					return MinRelocation;
				}
				else if(ans==-1){
	...
	if (depth == 0) {
		...
		total+=end-start;
		ans=branch_and_bound(q, UB,UB_cur, LB,both,DstDeque);
		if (ans!=0 && ans!=-1) {
			return MinRelocation;
		}
