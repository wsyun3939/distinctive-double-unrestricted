...
int branch_and_bound(IntDequeue *q, int UB, int UB_cur, int LB, direction Dir,int DstDeque) {
	...
	for(i=0;i<STACK;i++){
		...
		if (SecondPosition == q[0].min_idx) { //積み替え後，取り出しが可能な場合
			DirNext = both;
				...
				if (branch_and_bound(q_temp, UB, UB_cur, LB_temp, DirNext,DstDeque)) {
				...
				Array_print(q_temp);
						...
						if(branch_and_bound(q_temp, UB, UB_cur, LB_temp, DirNext,j)){
							Array_terminate(q_temp);
							free(q_temp);
							return MinRelocation;
						}
		...
		for(j=STACK-1;j>0;j--){
			if(BG_index[i][j]==0 && !(i==DstDeque && dir==Dir)){
				...
				if(branch_and_bound(q, UB, UB_cur, LB_temp, lower,j)){
					return MinRelocation;
				}
	...
	for(i=0;i<STACK;i++){
		...
		if (SecondPosition == q[0].min_idx) { //積み替え後，取り出しが可能な場合
			DirNext = both;
				...
				Array_print(q_temp);
						...
						if(branch_and_bound(q_temp, UB, UB_cur, LB_temp+1, DirNext,j)){
							Array_terminate(q_temp);
							free(q_temp);
							return MinRelocation;
						}
		...
		for(j=STACK-1;j>=0;j--){
			if(BG_index[i][j]==1 && !(i==DstDeque && dir==Dir)){
				...
				if(branch_and_bound(q, UB, UB_cur, LB_temp+1, lower,j)){
					return MinRelocation;
				}
				Deque(&q[j],&num_ret,dir);
	...
	if (depth == 0) {
		...
		if (branch_and_bound(q, UB,UB_cur, LB,both,DstDeque)) {
			return MinRelocation;
		}
		return -1;
	}
